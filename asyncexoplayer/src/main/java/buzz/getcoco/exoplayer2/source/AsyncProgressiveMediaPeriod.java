...
import buzz.getcoco.exoplayer2.ParserException;
import buzz.getcoco.exoplayer2.SeekParameters;
import buzz.getcoco.exoplayer2.decoder.DecoderInputBuffer;
import buzz.getcoco.exoplayer2.drm.DrmSessionEventListener;
import buzz.getcoco.exoplayer2.drm.DrmSessionManager;
import buzz.getcoco.exoplayer2.extractor.Extractor;
import buzz.getcoco.exoplayer2.extractor.ExtractorOutput;
import buzz.getcoco.exoplayer2.extractor.PositionHolder;
import buzz.getcoco.exoplayer2.extractor.SeekMap;
import buzz.getcoco.exoplayer2.extractor.TrackOutput;
import buzz.getcoco.exoplayer2.metadata.Metadata;
import buzz.getcoco.exoplayer2.metadata.icy.IcyHeaders;
import buzz.getcoco.exoplayer2.source.IcyDataSource;
import buzz.getcoco.exoplayer2.source.LoadEventInfo;
import buzz.getcoco.exoplayer2.source.MediaLoadData;
import buzz.getcoco.exoplayer2.source.MediaPeriod;
import buzz.getcoco.exoplayer2.source.MediaSourceEventListener;
import buzz.getcoco.exoplayer2.source.ProgressiveMediaExtractor;
import buzz.getcoco.exoplayer2.source.SampleQueue;
import buzz.getcoco.exoplayer2.source.SampleStream;
...
import buzz.getcoco.exoplayer2.upstream.DataSourceUtil;
import buzz.getcoco.exoplayer2.upstream.DataSpec;
import buzz.getcoco.exoplayer2.upstream.LoadErrorHandlingPolicy;
import buzz.getcoco.exoplayer2.upstream.Loader;
import buzz.getcoco.exoplayer2.upstream.StatsDataSource;
...
import buzz.getcoco.exoplayer2.util.ParsableByteArray;
...
public class AsyncProgressiveMediaPeriod implements MediaPeriod,
    ExtractorOutput,
    Loader.Callback<AsyncProgressiveMediaPeriod.ExtractingLoadable>,
  ...
  private final AsyncProgressiveMediaPeriod.Listener listener;
  ...
  private AsyncProgressiveMediaPeriod.TrackId[] sampleQueueTrackIds;
  private boolean sampleQueuesBuilt;

  private boolean prepared;
  private boolean haveAudioVideoTracks;
  private AsyncProgressiveMediaPeriod.TrackState trackState;
  ...
  public AsyncProgressiveMediaPeriod(
      Uri uri,
      ...
      AsyncProgressiveMediaPeriod.Listener listener,
      Allocator allocator,
    ...
    loader = new Loader("asyncProgressiveMediaPeriod");
        ...
        () -> {
          if (!released) {
            Assertions.checkNotNull(callback)
                .onContinueLoadingRequested(AsyncProgressiveMediaPeriod.this);
          }
        };
    handler = Util.createHandlerForCurrentLooper();
    sampleQueueTrackIds = new AsyncProgressiveMediaPeriod.TrackId[0];
  ...
  public long selectTracks(
    ...
    for (int i = 0; i < selections.length; i++) {
      if (streams[i] != null && (selections[i] == null || !mayRetainStreamFlags[i])) {
        int track = ((AsyncProgressiveMediaPeriod.SampleStreamImpl) streams[i]).track;
    ...
    for (int i = 0; i < selections.length; i++) {
      if (streams[i] == null && selections[i] != null) {
        ...
        streams[i] = new AsyncProgressiveMediaPeriod.SampleStreamImpl(track);
  ...
  public void onLoadCompleted(
      AsyncProgressiveMediaPeriod.ExtractingLoadable loadable, long elapsedRealtimeMs, long loadDurationMs) {
  ...
  public void onLoadCanceled(
      AsyncProgressiveMediaPeriod.ExtractingLoadable loadable, long elapsedRealtimeMs, long loadDurationMs, boolean released) {
    StatsDataSource dataSource = loadable.dataSource;
    LoadEventInfo loadEventInfo =
        new LoadEventInfo(
            loadable.loadTaskId,
  ...
  public Loader.LoadErrorAction onLoadError(
      AsyncProgressiveMediaPeriod.ExtractingLoadable loadable,
  ...
  public TrackOutput track(int id, int type) {
    return prepareTrackOutput(new AsyncProgressiveMediaPeriod.TrackId(id, /* isIcyTrack= */ false));
  }
  ...
  /* package */ TrackOutput icyTrack() {
    return prepareTrackOutput(new AsyncProgressiveMediaPeriod.TrackId(0, /* isIcyTrack= */ true));
  }
  ...
  private TrackOutput prepareTrackOutput(AsyncProgressiveMediaPeriod.TrackId id) {
    int trackCount = sampleQueues.length;
    for (int i = 0; i < trackCount; i++) {
    ...
    AsyncProgressiveMediaPeriod.TrackId[] sampleQueueTrackIds = Arrays
        .copyOf(this.sampleQueueTrackIds, trackCount + 1);
  ...
  private void maybeFinishPrepare() {
    ...
    trackState = new AsyncProgressiveMediaPeriod.TrackState(new TrackGroupArray(trackArray), trackIsAudioVideoFlags);
    prepared = true;
    Assertions.checkNotNull(callback).onPrepared(this);
  }

  private void copyLengthFromLoader(AsyncProgressiveMediaPeriod.ExtractingLoadable loadable) {
  ...
  private void startLoading() {
    AsyncProgressiveMediaPeriod.ExtractingLoadable loadable =
        new AsyncProgressiveMediaPeriod.ExtractingLoadable(
   ...
   *     retry.
   */
  private boolean configureRetry(AsyncProgressiveMediaPeriod.ExtractingLoadable loadable, int currentExtractedSampleCount) {
  ...
  private final class SampleStreamImpl implements SampleStream {
    ...
    public boolean isReady() {
      return AsyncProgressiveMediaPeriod.this.isReady(track);
    }

    @Override
    public void maybeThrowError() throws IOException {
      AsyncProgressiveMediaPeriod.this.maybeThrowError(track);
    }

    @Override
    public int readData(
        FormatHolder formatHolder, DecoderInputBuffer buffer, @ReadFlags int readFlags) {
      return AsyncProgressiveMediaPeriod.this.readData(track, formatHolder, buffer, readFlags);
    }

    @Override
    public int skipData(long positionUs) {
      return AsyncProgressiveMediaPeriod.this.skipData(track, positionUs);
    }
  }
  ...
  private static final class TrackId {
    ...
    public boolean equals(@Nullable Object obj) {
      ...
      AsyncProgressiveMediaPeriod.TrackId other = (AsyncProgressiveMediaPeriod.TrackId) obj;
